...
CDM_DIR = REPO_DIR / "cdm"
sys.path.insert(0, str(CDM_DIR))

from rgbddepth.dpt import RGBDDepth  # noqa: E402
...
def _require(modname: str, install_hint: str):
    try:
        ...
        raise SystemExit(
            ...
            f"  Install in your `manip-as-in-sim` env: {install_hint}\n"
        ) from e
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Real-time Manip-as-in-Sim CDM + D435i. Bench / AprilTag modes.",
    ...
    p.add_argument(
        "--model-path", type=str,
        default=str(REPO_DIR / "weights/cdm_d435.ckpt"),
        help="Path to the D435 CDM checkpoint.",
    )
    p.add_argument(
        "--encoder", choices=["vits", "vitb", "vitl", "vitg"], default="vitl",
        help="CDM ViT encoder size. The published D435 checkpoint is vitl.",
    )
    p.add_argument("--input-size", type=int, default=518)
    p.add_argument(
        "--max-depth", type=float, default=6.0,
        help="Maximum valid raw/predicted depth in meters; larger values become 0.",
    )
    p.add_argument("--inverse-depth-eps", type=float, default=1e-6)
...
def _load_cdm_state(model: torch.nn.Module, checkpoint_path: Path) -> None:
    checkpoint = torch.load(str(checkpoint_path), map_location="cpu")
    raw_state = _checkpoint_state(checkpoint)
    if not isinstance(raw_state, dict):
        raise TypeError(f"Unsupported checkpoint state in {checkpoint_path}")

    prefixes = ("module.", "model.", "pipeline.", "rgbddepth.")
    candidates = [
        raw_state,
        _strip_prefixes(raw_state, prefixes),
    ]
    model_keys = set(model.state_dict().keys())
    best = max(candidates, key=lambda s: sum(k in model_keys for k in s.keys()))
    matched = sum(k in model_keys for k in best.keys())
    if matched == 0:
        raise RuntimeError(f"No checkpoint keys matched RGBDDepth for {checkpoint_path}")

    msg = model.load_state_dict(best, strict=False)
    logging.info(
        f"Checkpoint key match: {matched}/{len(model_keys)} "
        f"(missing={len(msg.missing_keys)}, unexpected={len(msg.unexpected_keys)})"
    )


def _similarity_depth(raw_depth_m: np.ndarray, max_depth_m: float) -> np.ndarray:
    depth = np.nan_to_num(raw_depth_m, nan=0.0, posinf=0.0, neginf=0.0).astype(np.float32)
    depth[depth > max_depth_m] = 0.0
    simi = np.zeros_like(depth, dtype=np.float32)
    valid = depth > 0.0
    simi[valid] = 1.0 / depth[valid]
    return simi


def _cdm_depth_to_metric(pred_similarity: np.ndarray, args: argparse.Namespace) -> np.ndarray:
    pred_similarity = np.nan_to_num(
        pred_similarity, nan=0.0, posinf=0.0, neginf=0.0,
    ).astype(np.float32)
    valid = pred_similarity > args.inverse_depth_eps
    depth = np.zeros_like(pred_similarity, dtype=np.float32)
    depth[valid] = 1.0 / pred_similarity[valid]
    depth[(depth < 0.0) | (depth > args.max_depth) | ~np.isfinite(depth)] = 0.0
    return depth
...
def load_model(args: argparse.Namespace) -> LoadedModel:
    ...
    if not model_path.exists():
        raise FileNotFoundError(
            f"CDM checkpoint not found: {model_path}\n"
            "Download depth-anything/camera-depth-model-d435/cdm_d435.ckpt into "
            "weights/ or pass --model-path explicitly."
        )

    logging.info(f"Loading CDM model: encoder={args.encoder} checkpoint={model_path}")
    t0 = time.perf_counter()
    model = RGBDDepth(**MODEL_CONFIGS[args.encoder])
    _load_cdm_state(model, model_path)
    ...
    dummy_depth = np.full((h_in, w_in), 1.0, dtype=np.float32)
    dummy_simi = _similarity_depth(dummy_depth, args.max_depth)
    inputs, (h, w) = model.image2tensor(dummy_rgb, dummy_simi, args.input_size)
    inputs = inputs.to(device)
    with torch.inference_mode(), autocast(device.type, enabled=args.mixed_precision):
        pred = model.forward(inputs)
        _ = F.interpolate(pred[:, None], (h, w), mode="nearest")
...
class D435i:
    ...
    def read_rgbd(self, timeout_ms: int = 2000) -> Optional[Tuple[np.ndarray, np.ndarray]]:
        ...
        raw_depth_m[raw_depth_m > self.args.max_depth] = 0.0
...
def run_one_inference(
    ...
    simi_depth = _similarity_depth(raw_depth_scaled, args.max_depth)
    inputs, (h, w) = loaded.model.image2tensor(color_scaled, simi_depth, args.input_size)
    inputs = inputs.to(device)
    ...
    if device.type == "cuda":
        ...
        start.record()
        with torch.inference_mode(), autocast(device.type, enabled=args.mixed_precision):
            pred_similarity = loaded.model.forward(inputs)
            pred_similarity = F.interpolate(pred_similarity[:, None], (h, w), mode="nearest")[0, 0]
        end.record()
        torch.cuda.synchronize(device)
    ...
    pred_similarity_np = pred_similarity.detach().float().cpu().numpy()
    depth_pred = _cdm_depth_to_metric(pred_similarity_np, args)
    ...
    try:
        ...
        while not _QUIT_REQUESTED:
            ...
            depth_scaled, raw_depth_scaled, infer_ms = run_one_inference(
                loaded, color_bgr, raw_depth, args, device,
            )
