...
be checked in the ``depth-anything-v2`` Conda env without RealSense hardware
...
def _stub_run_inference(loaded, color_bgr, args, device):
    if args.scale != 1.0:
...
def _assert_record_sample(rec_dir: Path) -> None:
    ...
    for f in ("rgb.png", "disp.npy", "depth.npy", "overlay.png"):
        assert (sample / f).exists(), f"missing {sample}/{f}"


def main() -> int:
    tmp = Path(tempfile.mkdtemp(prefix="depth_anything_v2_rt_smoke_"))
