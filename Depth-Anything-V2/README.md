...
same CSV columns, recorded `rgb.png`, `disp.npy`, `depth.npy`, `overlay.png`,
and the same end-of-run summary. It uses the metric-depth model so
`depth.npy` and AprilTag errors are in meters.
...
Place the indoor metric checkpoint at:
...
```bash
mkdir -p checkpoints
python -c "from huggingface_hub import hf_hub_download; hf_hub_download(repo_id='depth-anything/Depth-Anything-V2-Metric-Hypersim-Large', filename='depth_anything_v2_metric_hypersim_vitl.pth', local_dir='checkpoints')"
```
...
python scripts/realtime_d435i.py \
  --mode bench \
  --log results/depth_anything_v2_bench.csv \
  --record-dir results/depth_anything_v2_bench_frames
```
