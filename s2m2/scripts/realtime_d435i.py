"""Real-time S2M2 on Intel RealSense D435i.

This mirrors ``Fast-FoundationStereo/scripts/realtime_d435i.py`` so S2M2
produces the same live HUD, CSV schema, record-dir artifacts, and AprilTag
ground-truth metrics.

Run::

    conda activate s2m2
    pip install pyrealsense2 pupil-apriltags   # one-time, if missing

    # bench mode
    python scripts/realtime_d435i.py --mode bench --model_type S \
        --log results/s2m2_bench.csv \
        --record-dir results/s2m2_bench_frames

    # apriltag mode (measure your printed tag's outer black border)
    python scripts/realtime_d435i.py --mode apriltag --tag-size 0.16 --tag-id 0 \
        --model_type S \
...
from s2m2.core.utils.image_utils import image_crop, image_pad  # noqa: E402
from s2m2.core.utils.model_utils import load_model as load_s2m2_model  # noqa: E402
...
def _require(modname: str, install_hint: str):
    try:
        ...
        raise SystemExit(
            ...
            f"  Install in your `s2m2` env: {install_hint}\n"
        ) from e
...
def _import_apriltags():
    return _require("pupil_apriltags", "pip install pupil-apriltags")
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Real-time S2M2 + D435i. Bench / AprilTag modes.",
    ...
    p.add_argument(
        "--weights-dir", "--weights_dir", type=str,
        default=str(REPO_DIR / "weights/pretrain_weights"),
        help="Directory containing S2M2 CH* pretrain .pth files.",
    )
    p.add_argument(
        "--model-type", "--model_type", dest="model_type",
        choices=["S", "M", "L", "XL"], default="S",
        help="S2M2 model size.",
    )
    p.add_argument(
        "--num-refine", "--num_refine", dest="num_refine",
        type=int, default=3,
        help="Number of local iterative refinement steps.",
    )
    p.add_argument(
        "--torch-compile", "--torch_compile", dest="torch_compile",
        action="store_true",
        help="Apply torch.compile to the S2M2 model.",
    )
    p.add_argument(
        "--allow-negative", "--allow_negative", dest="allow_negative",
        action="store_true",
    ...
    p.add_argument(
        ...
        help="IR projector. 'off' is in-distribution for S2M2.",
    )
    p.add_argument(
...
def load_model(args: argparse.Namespace) -> LoadedModel:
    ...
    logging.info(
        f"Loading S2M2 {args.model_type} from {args.weights_dir} "
        f"(num_refine={args.num_refine}, allow_negative={args.allow_negative})"
    )
    model = load_s2m2_model(
        args.weights_dir,
        args.model_type,
    ...
    if model is None:
        raise RuntimeError(
            f"Failed to load S2M2 weights from {args.weights_dir} "
            f"for model_type={args.model_type}"
        )
    model = model.eval()
    if args.torch_compile:
        logging.info("Compiling S2M2 with torch.compile ...")
    ...
    if h_in < 32 or w_in < 32:
        raise ValueError(f"Scaled input is too small for S2M2: {w_in}x{h_in}")
    ...
    dummy_l = image_pad(dummy_l, 32).to(device)
    dummy_r = image_pad(dummy_r, 32).to(device)
...
class D435i:
    ...
    def start(self) -> CamCalib:
        ...
        cfg = rs.config()
        cfg.enable_stream(
            rs.stream.infrared, 1, self.args.width, self.args.height,
            rs.format.y8, self.args.fps,
        )
        cfg.enable_stream(
            rs.stream.infrared, 2, self.args.width, self.args.height,
            rs.format.y8, self.args.fps,
        )
...
def _ir_to_tensor(ir: np.ndarray, scale: float, device: torch.device) -> torch.Tensor:
    ...
    if ir.ndim == 2:
        ...
        ir3 = ir[..., :3]
    t = torch.from_numpy(np.ascontiguousarray(ir3)).to(device).float()
...
def run_one_inference(
    ...
    """Returns (disparity_px, depth_placeholder, infer_ms)."""
    left_t = _ir_to_tensor(ir_left, args.scale, device)
    right_t = _ir_to_tensor(ir_right, args.scale, device)
    H, W = left_t.shape[-2:]

    left_t = image_pad(left_t, 32).to(device)
    right_t = image_pad(right_t, 32).to(device)
    ...
    if device.type == "cuda":
        ...
        start.record()
        with torch.inference_mode(), _autocast_context(device):
            pred_disp, _pred_occ, _pred_conf = loaded.model(left_t, right_t)
        end.record()
        torch.cuda.synchronize(device)
    ...
    pred_disp = image_crop(pred_disp.float(), (H, W)).squeeze()
    disp_np = (
        pred_disp.detach().cpu().numpy().reshape(H, W).clip(0, None).astype(np.float32)
    )
...
def reprojection_error_px(
    ...
    Evaluates both tag-local y-axis sign conventions and keeps the smaller
    residual, matching the FFS runner and avoiding the known corner-ordering
    false-positive.
    """
    s = tag_size_m / 2.0
    R = obs.pose_R
