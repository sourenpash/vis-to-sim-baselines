...
be checked in the ``stereoanywhere`` Conda env without RealSense hardware or
...
def _stub_run_inference(loaded, ir_left, ir_right, args, device):
    ...
    return disp, np.empty(0, dtype=np.float32), 6.0
...
def main() -> int:
    tmp = Path(tempfile.mkdtemp(prefix="stereoanywhere_rt_smoke_"))
    ...
    if os.environ.get("STEREOANYWHERE_RT_SMOKE_KEEP") != "1":
