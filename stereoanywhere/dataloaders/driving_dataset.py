...
class DrivingDataset(BaseDataset):
    def load_data(self, datapath):
        image2_list = sorted(glob(osp.join(datapath, 'frames_cleanpass/*/*/slow/left/*.png')))
        image3_list = sorted(glob(osp.join(datapath, 'frames_cleanpass/*/*/slow/right/*.png')))
        gt2_list = sorted(glob(osp.join(datapath, 'disparity/*/*/slow/left/*.pfm')))
        gt3_list = sorted(glob(osp.join(datapath, 'disparity/*/*/slow/right/*.pfm')))

        if self.mono is not None:
            image2_mono_list = sorted(glob(osp.join(datapath, f'frames_cleanpass/*/*/slow/left_{self.mono}/*.png')))
            image3_mono_list = sorted(glob(osp.join(datapath, f'frames_cleanpass/*/*/slow/right_{self.mono}/*.png')))
