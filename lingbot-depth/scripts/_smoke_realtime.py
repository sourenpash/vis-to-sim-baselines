...
Patches the D435i camera, model load, and inference call so the live runner
can be tested in the ``lingbot-depth`` Conda env without a camera, GPU, or
downloaded model.
"""
...
class _FakeCamera:
    def __init__(self, args) -> None:
        ...
        self._color = np.stack(
            [
                (xx % 255).astype(np.uint8),
                (yy % 255).astype(np.uint8),
                (((xx + yy) // 2) % 255).astype(np.uint8),
            ],
            axis=-1,
        )
    ...
    def read_rgbd(self, timeout_ms: int = 2000) -> Tuple[np.ndarray, np.ndarray]:
        noise = self._rng.normal(0, 0.002, size=self._depth.shape).astype(np.float32)
        return self._color.copy(), np.clip(self._depth + noise, 0, None)

    def stop(self) -> None:
        pass


def _stub_load_model(args, calib) -> rt.LoadedModel:
...
def _stub_run_inference(loaded, color_bgr, raw_depth_m, calib, args, device):
    color_scaled, raw_depth_scaled = rt._resize_inputs(color_bgr, raw_depth_m, args.scale)
    refined = raw_depth_scaled + 0.05
    return refined.astype(np.float32), raw_depth_scaled.astype(np.float32), 7.0
...
def main() -> int:
    tmp = Path(tempfile.mkdtemp(prefix="lingbot_rt_smoke_"))
    ...
    if os.environ.get("LINGBOT_RT_SMOKE_KEEP") != "1":
